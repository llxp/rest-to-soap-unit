...
PID_FILE=/var/run/unit/control.unit.sock
WORKER_PID_FILE=/var/run/unit/unitd.pid

curl_put()
{
    RET=`/usr/bin/curl -s -w '%{http_code}' -X PUT --data-binary @$1 --unix-socket ${PID_FILE} http://localhost/$2`
    RET_BODY=${RET::-3}
...
if [ "$1" = "unitd" -o "$1" = "unitd-debug" ]; then
    if /usr/bin/find "/var/lib/unit/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        if /usr/bin/find "/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
            ...
            /usr/sbin/unitd --control unix:${PID_FILE} --pid ${WORKER_PID_FILE} --log /var/log/unit/unitd.log

            while [ ! -S ${PID_FILE} ]; do echo "$0: Waiting for control socket to be created..."; /bin/sleep 0.1; done
            # even when the control socket exists, it does not mean unit has finished initialisation
            # this curl call will get a reply once unit is fully launched
            /usr/bin/curl -s -X GET --unix-socket ${PID_FILE} http://localhost/
            ...
            kill -TERM `/bin/cat ${WORKER_PID_FILE}`

            while [ -S ${PID_FILE} ]; do echo "$0: Waiting for control socket to be removed..."; /bin/sleep 0.1; done
...
/usr/sbin/unitd --no-daemon --control unix:${PID_FILE} --pid ${WORKER_PID_FILE} --log /var/log/unit/unitd.log